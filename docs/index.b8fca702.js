...
function render() {
    ...
    let k = 0;
    for(let i = 0; i < game.size; i++)for(let j = 0; j < game.size; j++, k++){
        const v = state[i][j];
        const cell = cells[k];
        cell.className = "field-cell";
        if (v) cell.classList.add(valueToClass(v));
 ...
 */ class Game {
   ...
   */ constructor(initialState){
    ...
    _processLineLeft(line) {
        ...
        let gained = 0;
        let i = 0;
        while(i < compact.length)if (i + 1 < compact.length && compact[i] === compact[i + 1]) {
    ...
    moveUp() {
        ...
        for(let c = 0; c < this.size; c++){
            ...
            for(let r = 0; r < this.size; r++)this.board[r][c] = nextLine[r];
            if (moved) anyMoved = true;
            gainedTotal += gained;
        }
        if (anyMoved) {
    ...
    moveDown() {
        ...
        for(let c = 0; c < this.size; c++){
            ...
            for(let r = 0; r < this.size; r++)this.board[r][c] = restored[r];
   ...
   */ start() {
        ...
        this.score = 0;
        this.moves = 0;
        for(let r = 0; r < this.size; r++)this.board[r].fill(0);
        this.insert();
        this.insert();
    }
    /**
...
exports.default = Game; // для браузера (ESM)
if (0, module.exports) module.exports = Game; // для Jest/Node (CommonJS)
